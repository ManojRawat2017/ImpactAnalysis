...
def create_NPSdataframe(data_file, build_file):
    ...
    iw3.loc[iw3['Tenure']<91,'TenureBucket'] = '1-3 mos'
    iw3.loc[iw3['Tenure'].between(91,180,inclusive=True),'TenureBucket'] = '4-6 mos'
    iw3.loc[iw3['Tenure'].between(180,365,inclusive=False),'TenureBucket'] = '7-12 mos'
    iw3.loc[iw3['Tenure'].between(365,730,inclusive=True),'TenureBucket'] = '1-2 yrs'
    iw3.loc[iw3['Tenure']>730,'TenureBucket'] = '2yrs +'
...
def make_pivot_table_new(df, interval,previous, current, value, dim=None):
    if dim == None: #AllUp Table, with No Splits by Levels of another Variable
        ...
        table[('Test', 'MoE')] = 1.65*100*((abs(table[('mean',previous)]/100)*(1-abs(table[('mean',previous)]/100)))/(table[('len',previous)]-1))**0.5
...
def custom_impact_pivot_allup(df, time_interval,previous,current):
    ...
    commerce_list.append('TenureBucket')
    commerce_list.extend(list(df.columns[df.columns.str.endswith('PaidSeatsBucket')]))
