...
import AdminImpact, EndUserImpact90CL
    ...
    def EndUser(self, data_file, build_file, survey="Default (Both)"):
        ...
        if self.kind == 'EndUser':
            if self.survey=='App': #No Suite Data 
                df = EndUserImpact90CL.create_NPSdataframe(data_file,build_file)
                EndUserImpact90CL.custom_impact_pivot_levels(df,self.time,self.survey,self.prev,self.curr)
            else:
                df = EndUserImpact90CL.create_NPSdataframe(data_file,build_file)
                EndUserImpact90CL.custom_impact_pivot_allup(df,self.time,self.prev,self.curr)
                EndUserImpact90CL.custom_impact_pivot_levels(df,self.time,self.survey,self.prev,self.curr)
